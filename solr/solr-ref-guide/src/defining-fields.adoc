...
`name`::
The name of the field. Field names should consist of alphanumeric or underscore characters only and not start with a digit. This is not currently strictly enforced, but other field names will not have first class support from all components and back compatibility is not guaranteed. Names with both leading and trailing underscores (e.g., `\_version_`) are <<reserved-field-names,reserved>>. Every field must have a `name`.
...
These fields do not always need to be present, but some functionality depends on them being present and expects known declarations to funtion properly. Do not create your own fields matching those names or, in general, fields with leading and trailing underscores.

- **+_version_+** - used for <<updating-parts-of-documents.adoc#in-place-updates,In-Place Updates and Optimistic Concurrency>>, as well as for <<solrcloud-with-legacy-configuration-files.adoc#solrcloud-with-legacy-configuration-files,SolrCloud>>
- **+_root_+**, **+_nest_path_+**, and **+_nest_parent_+** - used for <<indexing-nested-documents.adoc#schema-configuration,indexing nested documents>>. 

// **+_ is confusing. we want '_' to be part of text, not italic, so we need to escape it with passthrough (+...+) and only then apply bold (**)
